                    ...
                    <form method="POST" action="{{ route('admin.user.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="card-body">
                            <div class="d-flex align-items-start align-items-sm-center gap-4">
                                @if($user && $user->avatar)
                                    <img src="{{ asset('images/' . $user->avatar) }}" alt="{{ $user->username }}" height="100" width="100" id="fileUpload" />
                                @else
                                    <img src="{{ asset('images/default-avatar.png') }}" alt="Default Avatar" height="100" width="100" id="fileUpload" />
                                @endif
                            ...
                            @error('avatar') <div class="text-danger mt-2">{{ $message }}</div> @enderror
                        </div>
...
<script>
    function resetForm() {
        $('#userForm')[0].reset();
        $("#fileUpload").attr("src", "{{ asset('images/default-avatar.png') }}");
    }
