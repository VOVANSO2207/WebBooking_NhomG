                                                ...
                                                @else
                                                    <div class="swiper-slide">
                                                        ...
                                                        @if (file_exists(public_path('images/' . $firstImage->image_url)))
                                                            <img src="{{ asset('images/' . $firstImage->image_url) }}"
                                                                alt="Room Image"
            ...
            hotelDetailRows.forEach(row => {
                row.addEventListener('click', function() {
                    ...
                    fetch(`/hotels/${currentHotelId}/detail`)
                        ...
                        .then(hotel => {
                            document.getElementById('modalHotelName').innerText = hotel
                                .hotel_name;
                            document.getElementById('modalHotelLocation').innerText = hotel
                                .location;
                            document.getElementById('modalHotelCity').innerText = hotel.city;
                            document.getElementById('modalHotelDescription').innerHTML = hotel
                                .description.length > 50 ? hotel.description.substring(0, 50) +
                                '...' : hotel.description;
                            document.getElementById('modalHotelRating').innerText = hotel
                            .rating;

                            const editRoute =
                                "{{ route('hotel.edit', ['hotel_id' => ':hotel_id']) }}";
                            document.getElementById('editHotelButton').setAttribute('href',
                                editRoute.replace(':hotel_id', currentHotelId));
                            ...
                            hotel.rooms.forEach(room => {
                                ...
                                roomItem.textContent =
                                    `${room.room_name}: ${room.price} VNĐ`;
