...
.filter .form-group {
    display: flex;
    flex-direction: column; /* Đảm bảo label và select xếp dọc */
    gap: 8px;
}
...
@media (min-width: 768px) {
    ...
    .filter .form-select {
        width: auto;
        flex: 0 1 300px; /* Giới hạn kích thước dropdown */
    }
}
.btn {
...
.btn:focus {
    outline: none;
    background-color: #004085; /* Màu tối hơn khi focus */
    box-shadow: 0 0 4px rgba(0, 123, 255, 0.5);
}

.btn:active {
    background-color: #00376d; /* Màu tối hơn khi nhấn */
    box-shadow: inset 0 3px 6px rgba(0, 0, 0, 0.3);
}

.btn-sm {
    padding: 6px 12px; /* Kích thước nhỏ hơn */
    font-size: 12px;
}
...
<section class="all-hotels py-5">
    ...
    <div class="form-group">
        ...
        <select name="sort_by" id="sort_filter" class="form-select" onchange="this.form.submit()">
            ...
            <option value="price_asc" {{ request('sort_by') == 'price_asc' ? 'selected' : '' }}>Giá từ thấp đến cao</option>
            <option value="price_desc" {{ request('sort_by') == 'price_desc' ? 'selected' : '' }}>Giá từ cao đến thấp</option>
            <option value="stars_desc" {{ request('sort_by') == 'stars_desc' ? 'selected' : '' }}>Hạng sao từ cao đến thấp</option>
            <option value="reviews_count" {{ request('sort_by') == 'reviews_count' ? 'selected' : '' }}>Được đánh giá nhiều</option>
        </select>
    </div>
</form>
